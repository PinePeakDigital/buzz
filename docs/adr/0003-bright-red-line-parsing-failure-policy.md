...
**Correction (2026-06-11):** the original validator also required times to *strictly* increase (`curT > prevT`), and that sub-rule was the one invariant the audit above never checked. It is wrong. Re-auditing the same 60-goal account read-only found **3208 rows across 52/60 goals** where a row shares its predecessor's exact timestamp — a *vertical step*, the line jumping instantaneously (typically a rate-row immediately followed by a value-row at the same instant, e.g. `[t, null, 0.1], [t, 2.4, null]`). These are legitimate and common; the strict gate blanked the chart on nearly every goal with the banner "time must be greater than the previous row time". **Zero** rows had a strictly *earlier* time. So the rule is corrected to non-decreasing: equal times (vertical steps) are valid and materialise as zero-duration segments (`valueAt` is left-continuous across the step — pre-jump value at the instant, post-jump line just after; `slopePerDayAt` skips zero-duration segments, so the value-step's inert 0 slope is never reported as a real rate — including when a step is the road's first segment, which 3/60 goals exhibit). The value-row slope is left 0 only to avoid a divide-by-zero. A strictly *earlier* time remains malformed and surfaced.

**Correction (2026-06-25):** "a strictly earlier time remains malformed and surfaced" was too broad — it assumed `roadall[0]` is always the earliest row. It isn't. Goal `soktid` has `roadall[0]` = `[t, 0, null]` as the anchor but `roadall[1]` = `[t-12h, null, 0]` *before* it, so `buzz review` blanked its chart with "time must not be earlier than the previous row time". **Root cause** (confirmed from the snapshot, not merely correlated): Beeminder builds `roadall` as `[tini, vini, null] + road + [terminal]`, prepending a synthetic anchor at the goal's `tini`. soktid's user-defined `road` kinks and its `initday` all sit on a clean daily grid (16:00 UTC, the current −11h deadline), but `tini` sits 12h off that grid (04:00 UTC) — so the first kink predates the anchor. The 12h offset is the signature of a **deadline change after the kinks were set**: the kinks (and `initday`) recompute onto the new deadline grid while `tini` stays snapped to the old one. The *negative* deadline is incidental — 53/54 negative-deadline goals in the account have an on-grid `tini` and parse fine; what matters is `tini` disagreeing with the kink grid, which only a post-creation deadline change produces. This is not corruption: Beeminder's own resolved `fullroad` **drops the pre-anchor knot** and starts the line at the anchor (`fullroad[0]` = `initday`; no row at the earlier instant). So the rule is narrowed to: a *leading* row earlier than the anchor (before any forward segment exists) is dropped, mirroring `fullroad`; a row earlier than its predecessor *after* a forward segment has been built remains malformed and surfaced (a genuine out-of-order/negative-duration segment). 1/60 goals in the account exhibits the leading case; it is rare but legitimate.
