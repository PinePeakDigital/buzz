...
- Esc is **locked while a form's write is in flight** (`datapoint.submitting` / `createGoal.creating`): you cannot close a busy form. This prevents closing a busy form, reopening a fresh one, and firing a second non-idempotent write before the first returns — and it keeps `modeGoalDetail`/`modeBrowse` from ever coexisting with an in-flight submit, so the per-handler "busy" checks live in exactly one place (the Esc ladder and the form-input guards) rather than being duplicated across every transition.
